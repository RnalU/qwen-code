...
interface ZhipuKeyPromptProps {
  onSubmit: (apiKey: string, model?: string) => void;
  onCancel: () => void;
}

export function ZhipuKeyPrompt({
  onSubmit,
  onCancel,
}: ZhipuKeyPromptProps): React.JSX.Element {
  ...
  useInput((input, key) => {
    ...
    if (input.includes('\n') || input.includes('\r')) {
      if (currentField === 'apiKey') {
        ...
        if (apiKey.trim()) {
          onSubmit(apiKey.trim(), model.trim() || undefined);
        } else {
  ...
  return (
    ...
    >
      ...
      <Box marginTop={1}>
        <Text>
          Please enter your Zhipu AI configuration. You can get an API key from{' '}
